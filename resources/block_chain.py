...
def encrypt(plaintext, inner_txt, key):
    ...
    block_size = 16
    plaintext = pad(modified_plaintext, block_size) # L + secret + R + padding(its length repeated)
    key_hash = hashlib.sha256(key).digest()

    ciphertext = b''

    for i in range(0, len(plaintext), block_size): # len(plaintext) // 16 times, which is an integer since its padded to block size
        block = plaintext[i:i + block_size] # 16 characters
        cipher_block = xor_bytes(block, key_hash) # XORed with key hash
        ciphertext += cipher_block # same order as plaintext
